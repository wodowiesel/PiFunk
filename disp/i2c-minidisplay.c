...
https://www.amazon.de/IZOKEE-Display-Raspberry-Mikrocontroller-Weiß-IIC/dp/B076PL474K 
e I2C-Adresse vom SA0 Input bestimmt: SA0=0 I2C-Adr=$78 / SA0=1 I2C-Adr=$7A. SA0 ist auf der Rückseite der Platine zugänglich 
um i2c treiber zu laden. 
chmod 666 /dev/i2c-1
#und/oder adduser pi i2c
#vcc an 3,3v
#gnd an gnd
#sda an pin 3
#scl an pin 5
#(rpi b+)
Display dunkel 1 mA
Display zu 10% hell 8 mA
Display zu 50% hell 20 mA
Display komplett hell 25 mA
#mit i2cdetect -l zeigt er liste der busse an
#dann mit i2cdetect -y 1 (wenn bus 1 sonst bus 0) liste er geräte auf dem bus auf
...
/*
Ich verwende u8glib oder adafruit lib. i2c addresse ist 0X3C. Läuft mit 3.3V und 5V. Lieferung war innerhalb von 9 Tagen aus China. LCD macht einen extrem hochwertigen Eindruck und nichts war defekt. Der Controller ist ein SSD_1306 mit 128x64 Pixel. 
#kompilieren mit: g++ wpi-i2c.c -o wpi-i2c -lwiringPi
#zuerst wird das display initialisiert, dann gecleared. mit reset_pos() wird die schreibeposition wieder auf anfang gesetzt auf dem display, dazu bitte die #dokumentation lesen, insbesondere das kapitel über die verschiedenen schreibmodi (vertikal/horizontal) etc. 
#Mit render() werden dann 8 Byte daten geschrieben, mit denen sich ein 8x8 px großes zeichen darstellen lässt, im beispiel ein kleines haus-symbol. im internet #gibt es etliche 8x8 header only fonts, mit denen sich dann einfach texte darstellen lassen.
*/